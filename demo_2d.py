...
from qwave.potentials import check_on_grid, double_slit, rect_barrier_1d
...
def run(V, psi0, snapshot_times, dt=0.01):
    s = Solver(g, V, dt, absorber=absorbing_mask(g, dt))
    psi, frames = psi0, []
...
psi0 = gaussian_packet(g, (-18, 0), (2.5, 4.0), (4.0, 0))
times = [0, 3.0, 5.0, 8.0]
frames = run(V, psi0, times)
...
# animation
s = Solver(g, V, 0.01, absorber=absorbing_mask(g, 0.01))
state = {"psi": psi0}
...
V = np.where((x >= -a / 2) & (x < a / 2), V0, 0.0)        # exactly 8 cells = width 1.0
...
def transmitted_1d(dx, dt, t_end=6.0):
    n = int(1024 / dx)
    g1 = Grid(n, n * dx)
    s1 = Solver(g1, rect_barrier_1d(g1, V0, a), dt)
...
print(f"2D transmitted             = {right:.5f}")
print(f"1D, same dx/dt             = {T_same:.5f}   (|diff| = {abs(right - T_same):.1e}  -> 2D code OK)")
print(f"1D, dx=1/64, dt=0.001      = {T_fine:.5f}")
print(f"analytic (k-averaged)      = {T_exact:.5f}   (demo-grid error {abs(right - T_exact) / T_exact:.1%})")
