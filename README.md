...
**Status:** the Python reference implementation (`qwave` 0.3.0), its validation and the reference data for the port are complete. Next, the solver will be ported to C++ → WebAssembly + WebGL.
...
- 24 checks against analytic results ([validation](docs/validation.md)), and golden files for the C++/WASM port ([porting](docs/porting.md))
...
reference/    golden files for the port (export_reference.py)
...
**現状:** Python によるリファレンス実装（`qwave` 0.3.0）、その数値検証、移植用のリファレンスデータが完成した段階です。今後 C++ → WebAssembly + WebGL へ移植します。
...
- 解析解との 24 項目の比較（[数値検証](docs/validation.md#japanese)）と、C++/WASM 移植用のリファレンスデータ（[移植](docs/porting.md#japanese)）
